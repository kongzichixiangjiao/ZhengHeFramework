...
class PXRequest: ZHReuqest {
    ...
    func requestPX(baseUrlType: ZHUrlType? = .url, zhUrlApi api: ZHRequestApi, method: HTTPMethod = .post, parameters: Parameters? = nil, encoding: ParameterEncoding = JSONEncoding.default, queue: DispatchQueue = DispatchQueue.main, successHandler: @escaping (ZHModel) -> (), errorHandler: @escaping (ZHReuqestError, ZHReuqestErrorCode) -> ()) {
        ...
        let _ = request(baseUrlType: baseUrlType, zhUrlApi: api.rawValue, method: method, parameters: parameters, encoding: encoding, headers: headers as? HTTPHeaders, queue: queue, successHandler: {
            [weak self] model in

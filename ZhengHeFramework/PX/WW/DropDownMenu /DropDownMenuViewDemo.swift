...
class DropDownMenuViewDemo: UIViewController {
    ...
    lazy var dropList:DropDownMenuView = {
        //传入一个二维数组即可
        let drop = DropDownMenuView.init(frame: CGRect.init(x: 0, y: 100, width: DropDownMenuViewDemo_kW, height: 40), tableArr: [self.moneyArray,self.limitArray,self.sortArray], selectClosure: { (com, row) in
            //tag是第几个标题菜单，row是菜单第几行
            print(com,row)
        })
        return drop
    }()
    ...
    override func viewWillDisappear(_ animated: Bool) {
        ...
        dropList.myMaskView.removeFromSuperview()
    }
    
    deinit {
        print("DropDownMenuViewDemo - deinit")
    }
